 ...
 */
	...
	*/
	...
	<div class="wrap">
        ...
        <h2><?php _e( 'Custom Logo', 'wpblink_metro' ); ?></h2>
        ...
        <form id="form-customlogo-options" action="options.php" method="post" enctype="multipart/form-data">
            ...
            ?>
            <p class="submit">
                <input name="theme_customlogo_options[submit]" id="submit_options_form" type="submit" class="button-primary" value="<?php esc_attr_e('Save Settings', 'wpblink_metro'); ?>" />
                <input name="theme_customlogo_options[reset]" type="submit" class="button-secondary" value="<?php esc_attr_e('Reset Defaults', 'wpblink_metro'); ?>" />		
            </p>
        </form>
    </div>
	<?php
	...
	function customlogo_options_settings_init() {
		...
		add_settings_section('customlogo_settings_header', __( 'Logo Options', 'wpblink_metro' ), 'customlogo_settings_header_text', 'customlogo');
		add_settings_field('customlogo_setting_logo',  __( 'Logo', 'wpblink_metro' ), 'customlogo_setting_logo', 'customlogo', 'customlogo_settings_header');
		add_settings_field('customlogo_setting_logo_preview',  __( 'Logo Preview', 'wpblink_metro' ), 'customlogo_setting_logo_preview', 'customlogo', 'customlogo_settings_header');
	}
	...
	?>
		<p><?php _e( 'You can upload any custom image to use as your logo.', 'wpblink_metro' ); ?></p>
		<p><b><?php _e( '(Rectangular images for best results).', 'wpblink_metro' ); ?></b></p>
	<?php
	}
	...
	?>
		...
		<input id="upload_logo_button" type="button" class="button" value="<?php _e( 'Upload Logo', 'wpblink_metro' ); ?>" />
			<?php if ( '' != $customlogo_options['logo'] ): ?>
				<input id="delete_logo_button" name="theme_customlogo_options[delete_logo]" type="submit" class="button" value="<?php _e( 'Delete Logo', 'wpblink_metro' ); ?>" />
			<?php endif; ?>
		<?php
	}
